...
/**
 * MariaDB database-based storage implementation.
 * Connects to a remote MariaDB database.
 */
public class MariaDBStorage extends SQLStorage {

    private final Config config;

    public MariaDBStorage(dev.cosax.cSXLandManager.CSXLandManager plugin, Config config) {
        super(plugin);
        this.config = config;
    }
    ...
    protected String getJdbcUrl() {
        return String.format("jdbc:mariadb://%s:%d/%s?useSSL=false&allowPublicKeyRetrieval=true",
                config.getDbHost(),
                config.getDbPort(),
